...
class deliverysetcheck_oxdeliverysetlist extends deliverysetcheck_oxdeliverysetlist_parent{
    public function getDeliverySetData( $sShipSet, $oUser, $oBasket )
    {
        ...
        if ($this->count() ) {
            ...
            $deldbg .= '<font color="red">'.oxLang::getInstance()->translateString('DELIVERYSETCHECK_SHIPPINGMETHODS').'</font> ';
            ...
            foreach ( $this as $sShipSetId => $oShipSet ) {
                ...
                if ( count( $aPaymentList ) ) {
                    ...
                    if ( $oDelList->hasDeliveries( $oBasket, $oUser, $oUser->getActiveCountry(), $sShipSetId ) ) {
                        ...
                        $deldbg .= oxLang::getInstance()->translateString('DELIVERYSETCHECK_SHIPPINGMETHOD_OK');
                    }
                    else $deldbg .= oxLang::getInstance()->translateString('DELIVERYSETCHECK_SHIPPINGRULES_ERROR');
                }
                else $deldbg .= oxLang::getInstance()->translateString('DELIVERYSETCHECK_PAYMENTS_ERROR');
            }
        }
        else $deldbg .= oxLang::getInstance()->translateString('DELIVERYSETCHECK_ACTIVESHPPINGESET_ERROR');
